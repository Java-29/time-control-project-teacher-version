...
export const authorize = (arr:Record<string, string[]>) =>
    (req: AuthRequest, res:Response, next: NextFunction) => {
        ...
        if(!roles&&pathMethod==="POST/accounts/login"||
           // !roles&&pathMethod==="GET/docs" ||
             roles!.some(item => arr[pathMethod].includes(item))) //ToDo incorrect request for registration with user
            next();
